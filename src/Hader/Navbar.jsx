 
import { AiOutlineMenu } from 'react-icons/ai'
import { useState } from 'react'
import { Link } from 'react-router-dom'
 
// import avatarImg from '../../../assets/images/placeholder.jpg'
import useAuth from '../Hook/useAuth'

const Navbar = () => {
  const { user, logOut } = useAuth()
  const [isOpen, setIsOpen] = useState(false)

  return (
    <div className='fixed md:container lg:container mx-auto '>
      <div className='py-4   border'>
       
       
          <div className='flex  items-center justify-between gap-3 md:gap-0'>
            {/* Logo */}
            <Link to='/'>
              <img
                className='hidden md:block lg:ml-15 rounded-full'
                src='https://i.ibb.co.com/v69px8SG/clubPic.jpg'
                alt='logo'
                width='100'
                height='100'
              />
            </Link>
            {/* Dropdown Menu */}
            <div className='relative lg:mr-75'>



              <div className='flex flex-row  items-center gap-3'>
                {/* Become A Host btn */}
                <div className='hidden md:block'>
                  {!user && (
                    <button
                      disabled={!user}
                      className='disabled:cursor-not-allowed cursor-pointer  py-3 px-4 text-sm font-semibold rounded-full  transition'
                    >
                      Host your home
                    </button>
                  )}
                </div>
                {/* Dropdown btn */}
                <div
                  onClick={() => setIsOpen(!isOpen)}
                  className='p-4 md:py-1 md:px-2 border-[1px]  flex flex-row items-center gap-3 rounded-full cursor-pointer hover:shadow-md transition'
                >
                  <AiOutlineMenu />
                  <div className='hidden md:block'>
                    {/* Avatar */}
                    <img
                      className='rounded-full'
                      referrerPolicy='no-referrer'
                      src={user && user.photoURL ? user.photoURL : ''}
                      alt='profile'
                      height='30'
                      width='30'
                    />
                  </div>
                </div>
              </div>
              {isOpen && (
                <div className='absolute rounded-xl shadow-md w-[40vw] md:w-[10vw]  overflow-hidden right-0 top-12 text-sm'>
                  <div className='flex flex-col cursor-pointer'>
                    <Link
                      to='/'
                      className='block md:hidden px-4 py-3  transition font-semibold'
                    >
                      Home
                    </Link>

                    {user ? (
                      <>
                          <Link
                      to='/dashboard'
                      className='block  px-4 py-3  transition font-semibold'
                    >
                    Dashboard
                    </Link>
                        <div
                          onClick={logOut}
                          className='px-4 py-3
                           transition font-semibold cursor-pointer'
                        >
                          Logout
                        </div>
                      </>
                    ) : (
                      <>
                        <Link
                          to='/SignIn'
                          className='px-4 py-3  transition font-semibold'
                        >
                          Sign IN
                        </Link>
                        <Link
                          to='/signup'
                          className='px-4 py-3  transition font-semibold'
                        >
                          Sign Up
                        </Link>
                      </>
                    )}
                  </div>
                </div>
              )}
            </div>
          </div>
        
      </div>
    </div>
  )
}

export default Navbar